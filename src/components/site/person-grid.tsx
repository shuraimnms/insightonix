...
export function PersonGrid({ people }: { people: PersonRow[] }) {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {people.map((m) => (
        <article key={m.id} className="group rounded-xl border border-border bg-card p-5 transition hover:border-brand/40 hover:shadow-elev">
          <div className="flex items-start gap-4">
            {m.photo_url ? (
              <img src={m.photo_url} alt={m.name} className="h-14 w-14 flex-none rounded-full object-cover" loading="lazy" />
            ) : (
            ...
            <div className="min-w-0">
              ...
              {m.title ? <div className="mt-0.5 text-xs uppercase tracking-wider text-brand font-semibold">{m.title}</div> : null}
              {m.affiliation ? (
                <div className="mt-1 flex items-start gap-1 text-sm text-muted-foreground">
                  <MapPin className="mt-0.5 h-3 w-3 flex-none" />
                  <span>{m.affiliation}{m.country ? `, ${m.country}` : ""}</span>
                </div>
              ) : null}
            </div>
          </div>
          {m.bio ? <p className="mt-4 line-clamp-4 text-sm leading-relaxed text-muted-foreground">{m.bio}</p> : null}
          {m.email ? (
