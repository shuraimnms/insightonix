...
export const Route = createFileRoute("/copyright-form")({
  head: () => ({
    meta: [
      { title: "Copyright Form — INSIGHTONIX" },
      {
        name: "description",
        content: "Fillable Copyright Assignment Form for INSIGHTONIX manuscripts.",
      },
    ],
...
function CopyrightForm() {
  ...
  const generatePDF = () => {
    ...
    if (!w) return;
    w.document
      .write(`<html><head><title>INSIGHTONIX Copyright Form — ${state.manuscriptTitle}</title>
