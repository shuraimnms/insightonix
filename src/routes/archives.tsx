  ...
  head: () => ({
    meta: [
      { title: "Archives (2015 – 2026) — INSIGHTONIX" },
      {
        name: "description",
        content:
          "Complete INSIGHTONIX publication archive — Volumes 1 through 12, quarterly issues (March, June, September, December) since 2015.",
      },
      { property: "og:title", content: "INSIGHTONIX Publication Archive (2015 – 2026)" },
      {
        property: "og:description",
        content:
          "Browse every volume and issue of INSIGHTONIX from Volume 1 (2015) to the current Volume 12 (2026).",
      },
    ],
        ...
        <section className="mb-10 rounded-2xl border border-brand/30 bg-gradient-to-br from-brand to-brand/80 p-6 text-brand-foreground shadow-elev md:p-8">
          <div className="grid gap-4 sm:grid-cols-4">
            {[
              ...
              <div
                key={s.l}
                className="rounded-lg bg-white/10 p-4 ring-1 ring-white/15 backdrop-blur-sm"
              >
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,3fr)_minmax(0,2fr)]">
              ...
              <div className="max-h-[720px] overflow-y-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {flatRows.map((r, idx) => (
                      <tr
                        key={`${r.year}-${r.volume}-${r.issue}`}
                        className={cn(
                          "border-t border-border/60 hover:bg-accent/60 transition-colors",
                      ...
                      >
                        ...
                        <td className="px-4 py-2">
                          <Link
                            to="/articles"
                            search={{ volume: r.volume } as never}
                            className="hover:text-brand hover:underline"
                          >
                            Volume {r.volume}
                          </Link>
                        ...
                        <td className={cn("px-4 py-2 font-semibold", MONTH_COLOR[r.month])}>
                          <Link
                            to="/articles"
                            search={{ volume: r.volume, issue: r.issue } as never}
          ...
          <section>
            <div className="rounded-xl border border-border bg-card overflow-hidden">
              <header className="border-b border-border bg-brand px-5 py-3 text-brand-foreground">
                <h2 className="font-serif text-lg font-semibold tracking-tight">
                  Archive Structure (Journal Stack)
                </h2>
              </header>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
                ...
                <ul className="mt-3 grid grid-cols-2 gap-2 text-sm">
                  {(["March", "June", "September", "December"] as const).map((m) => (
                    <li key={m} className="flex items-center gap-2">
                      ...
                      <span>
                        Issue {m === "March" ? 1 : m === "June" ? 2 : m === "September" ? 3 : 4} –{" "}
                        {m}
                      </span>
                    </li>
                  ))}
              ...
              <div className="rounded-xl border border-amber-400/40 bg-amber-50/50 p-4 dark:bg-amber-950/20">
                <div className="text-xs uppercase tracking-wider font-semibold text-amber-700 dark:text-amber-300">
                  Important Note
                </div>
                <p className="mt-2 text-sm text-muted-foreground">
                  The journal is published quarterly in March, June, September and December. This
                  archive is organised as per the standard volume-per-year system.
                </p>
              </div>
            </div>
          </section>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-serif text-base font-semibold">
                INSIGHTONIX is published <span className="text-brand">QUARTERLY</span> in March,
                June, September and December.
              </div>
              <div className="text-xs text-muted-foreground">
                2015 (started in June) → 2026 (up to June)
              </div>
            </div>
          </div>
          <Link
            ...
            .map((i) => (
              <li key={i.issue} className="relative py-1.5">
                ...
                <Link
                  ...
                  className={cn(
                    "group inline-flex items-center gap-2 text-sm font-medium hover:underline",
                    MONTH_COLOR[i.month],
                  )}
                >
