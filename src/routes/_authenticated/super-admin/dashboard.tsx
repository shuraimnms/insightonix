...
function Kpi({
  ...
  label: string;
  value: string | number;
  sub: string; // eslint-disable-next-line @typescript-eslint/no-explicit-any
  icon: any;
  tint: string;
}) {
  return (
...
function Dashboard() {
  ...
  const [activity, setActivity] = useState<
    Array<{ id: string; action: string; created_at: string; entity_type: string | null }>
  >([]);
  const [email, setEmail] = useState("");
  const [health, setHealth] = useState<{
    status: "healthy" | "warning" | "critical";
    memory: number;
  }>({ status: "healthy", memory: 42 });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setHealth({
        status: totalPapers > 0 ? "healthy" : "warning",
    ...
    <div className="space-y-8">
      <header>
        ...
        <p className="mt-2 text-sm text-slate-400">
          Welcome back, <span className="font-medium text-slate-200">{email}</span>! Here's what's
          happening in {activeSite?.name ?? "your system"}.
        </p>
      </header>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Kpi
          ...
          sub={`Active: ${stats?.activeUsers ?? 0}`}
          icon={Users}
        ...
        <Kpi
          ...
          sub={`Published: ${stats?.publishedPapers ?? 0}`}
          icon={FileText}
        ...
        <Kpi
          ...
          sub={`Avg rating: ${stats?.avgRating.toFixed(1) ?? "—"}/5`}
          icon={Eye}
        ...
        <Kpi
          ...
          sub={`↗ ${stats?.downloadsDelta ?? 0}% increase`}
          icon={Download}
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-6 lg:col-span-2">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-0.5 text-sm text-slate-400">
                Manage your system efficiently with these quick access tools.
              </p>
            </div>
            <div className="hidden rounded-full bg-amber-500/15 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-amber-400 sm:inline-block">
              Tools
            </div>
          </div>
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {[
              ...
              {
                to: "/super-admin/chief-patrons",
                label: "Chief Patrons",
                icon: Crown,
                tint: "from-rose-500 to-pink-600",
              },
              {
                to: "/super-admin/animations",
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    className={cn(
                      "flex h-9 w-9 items-center justify-center rounded-md bg-gradient-to-br text-white shadow",
                      a.tint,
                    )}
                  >
                ...
                activity.map((a) => (
                  <tr key={a.id} className="text-slate-300">
                    <td className="px-3 py-2">
                      <span className="rounded-full bg-slate-800 px-2 py-0.5 text-[11px] font-medium">
                        {a.action}
                      </span>
                    </td>
