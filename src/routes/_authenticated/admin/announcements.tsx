...
function Ann() {
  ...
  const save = async () => {
    if (!edit) return;
    const payload = { title: edit.title!, body: edit.body ?? "", is_published: !!edit.is_published, published_at: edit.published_at ?? new Date().toISOString() };
    const { error } = edit.id ? await supabase.from("announcements").update(payload).eq("id", edit.id) : await supabase.from("announcements").insert(payload);
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <div><div className="text-xs uppercase tracking-widest text-brand font-semibold">Content</div><h1 className="mt-1 font-serif text-3xl font-semibold">Announcements</h1></div>
        <button onClick={() => setEdit({ is_published: true })} className="inline-flex h-10 items-center gap-2 rounded-md bg-brand px-4 text-sm font-semibold text-brand-foreground"><Plus className="h-4 w-4" /> New</button>
      </div>
      <div className="mt-6 space-y-3">
        {rows.map((r) => (
          <div key={r.id} className="flex items-start justify-between rounded-xl border border-border bg-card p-5">
            <div className="min-w-0 flex-1">
              <div className="flex items-baseline gap-3">
                <span className="text-xs text-muted-foreground">{new Date(r.published_at).toLocaleDateString()}</span>
                {r.is_published ? null : <span className="rounded-full bg-secondary px-2 py-0.5 text-xs">draft</span>}
              </div>
      ...
      {edit && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 p-4" onClick={() => setEdit(null)}>
          <div className="w-full max-w-lg rounded-xl border border-border bg-popover p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between"><h2 className="font-serif text-xl font-semibold">{edit.id ? "Edit" : "New announcement"}</h2><button onClick={() => setEdit(null)}><X className="h-4 w-4" /></button></div>
            <div className="mt-4 space-y-3">
              ...
              <label className="block"><div className="mb-1 text-xs uppercase tracking-wider text-muted-foreground">Body</div><textarea rows={6} value={edit.body ?? ""} onChange={(e) => setEdit({ ...edit, body: e.target.value })} className="w-full rounded-md border border-border bg-background p-2 text-sm" /></label>
