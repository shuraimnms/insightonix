...
export const Route = createFileRoute("/_authenticated/admin/impact-factor")({
    ...
    <div>
      <header className="mb-6">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Yearly citation and h-index numbers shown on the Indexing page.
        </p>
      </header>
      ...
      <div className="mb-8 rounded-xl border border-border bg-card p-5">
        ...
        <div className="grid gap-3 md:grid-cols-6">
          <input
            ...
            placeholder="Year"
            value={d.year}
            onChange={(e) => setD({ ...d, year: Number(e.target.value) })}
          ...
          <input
            ...
            value={d.impact_factor}
            onChange={(e) => setD({ ...d, impact_factor: e.target.value })}
            className="h-10 rounded-md border border-border bg-background px-3 text-sm"
          />
          <input
            type="number"
            ...
            value={d.citations}
            onChange={(e) => setD({ ...d, citations: e.target.value })}
            className="h-10 rounded-md border border-border bg-background px-3 text-sm"
          />
          <input
            type="number"
          ...
          <input
            ...
            placeholder="h-index"
            value={d.h_index}
            onChange={(e) => setD({ ...d, h_index: e.target.value })}
            className="h-10 rounded-md border border-border bg-background px-3 text-sm"
          />
          <input
            placeholder="Source"
            ...
            <div key={r.id} className="rounded-xl border border-border bg-card p-5">
                ...
                <div>
                  ...
                  <div className="mt-1 flex items-baseline gap-2">
                    ...
                    <div className="font-serif text-3xl font-semibold">
                      {Number(r.impact_factor).toFixed(3)}
                    </div>
                  </div>
                </div>
                <button
                  onClick={() => remove(r.id)}
                  className="inline-flex h-8 items-center rounded-md border border-destructive/40 px-2 text-xs text-destructive"
                >
              ...
              <dl className="mt-4 grid grid-cols-3 gap-2 text-xs">
                <div>
                  ...
                  <dd className="font-semibold">{r.citations}</dd>
                </div>
                <div>
                  <dt className="text-muted-foreground">Pubs</dt>
                  <dd className="font-semibold">{r.publications}</dd>
                </div>
                <div>
                  <dt className="text-muted-foreground">h-index</dt>
                  <dd className="font-semibold">{r.h_index ?? "—"}</dd>
                </div>
              </dl>
              {r.source ? (
                <div className="mt-2 text-[11px] text-muted-foreground">Source: {r.source}</div>
              ) : null}
            </div>
          ))}
        </div>
