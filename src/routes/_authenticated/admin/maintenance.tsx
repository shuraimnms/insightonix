    ...
    <>
      <div>
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
          When enabled, visitors see a maintenance banner across every public page. Staff and admins
          bypass it and can continue working.
        </p>
      </div>
      ...
      <div className="mt-8 rounded-xl border border-border bg-card p-6">
        <div className="flex items-start gap-4">
          <div
            className={`rounded-full p-3 ${mode.enabled ? "bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-200" : "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-200"}`}
          >
            {mode.enabled ? (
          ...
          <div className="flex-1">
            <div className="font-serif text-lg font-semibold">
              {mode.enabled ? "Maintenance mode is ON" : "Site is live"}
            </div>
            ...
            <button
              onClick={save}
              disabled={saving}
              className="mt-4 rounded-md bg-brand px-4 py-2 text-sm font-semibold text-brand-foreground disabled:opacity-60"
            >
              {saving ? "Saving…" : "Save"}
            </button>
          </div>
