...
function Sub() {
  ...
  const exportCsv = () => {
    ...
    a.href = url;
    a.download = "insightonix-subscribers.csv";
    a.click();
    ...
    <>
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="mt-1 font-serif text-3xl font-semibold">Newsletter subscribers</h1>
        </div>
        <button
          onClick={exportCsv}
          disabled={rows.length === 0}
          className="inline-flex h-10 items-center gap-2 rounded-md bg-brand px-4 text-sm font-semibold text-brand-foreground disabled:opacity-60"
        >
