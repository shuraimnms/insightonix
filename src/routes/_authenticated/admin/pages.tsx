  ...
  const save = async () => {
    ...
    const { error } = await supabase
      ...
      .update({
        title: sel.title,
        content: sel.content,
        meta_description: sel.meta_description,
      })
      .eq("id", sel.id);
  ...
  return (
    <>
      <div>
        ...
        <h1 className="mt-1 font-serif text-3xl font-semibold">Editable pages</h1>
      </div>
          ...
          <ul className="space-y-1">
            {rows.map((r) => (
              <li key={r.id}>
                <button
                  onClick={() => setSel(r)}
                  className={`w-full text-left rounded-md px-3 py-2 text-sm ${sel?.id === r.id ? "bg-brand text-brand-foreground" : "hover:bg-accent"}`}
                >
                  {r.title}
            ...
            <div className="rounded-xl border border-border bg-card p-6">
              ...
              <label className="mt-3 block">
                ...
                <input
                  ...
                  onChange={(e) => setSel({ ...sel, meta_description: e.target.value })}
                  className="h-10 w-full rounded-md border border-border bg-background px-3 text-sm"
                />
              </label>
              <label className="mt-3 block">
                <div className="mb-1 text-xs uppercase tracking-wider text-muted-foreground">
                  Content (Markdown-ish: ## H2, ### H3, - list, **bold**)
                </div>
                <textarea
                  rows={22}
                  value={sel.content}
