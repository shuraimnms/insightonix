...
function IssuesAdmin() {
  ...
  const autoPage = async (id: string) => {
    ...
    try {
      const res = await paginate({
        data: { issue_id: id, pages_per_article: Number(perStr), start_page: Number(startStr) },
      });
  ...
  const save = async () => {
    if (!edit) return;
    const payload = {
      volume: edit.volume ?? 1,
      number: edit.number ?? 1,
    ...
    <>
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="mt-1 font-serif text-3xl font-semibold">Issues</h1>
        </div>
        <button
          onClick={() =>
            setEdit({ volume: 1, number: 1, year: new Date().getFullYear(), is_published: false })
          }
              ...
              <tr key={r.id}>
                ...
                <td className="p-3">
                  <div className="flex gap-1">
                    <button
                      onClick={() => autoPage(r.id)}
                      title="Auto-paginate articles"
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <label>
                  ...
                  <input
                    type="number"
                    value={edit.number ?? 1}
                    onChange={(e) => setEdit({ ...edit, number: Number(e.target.value) })}
                    className="h-10 w-full rounded-md border border-border bg-background px-3 text-sm"
                  />
                </label>
                <label>
                  ...
                  <input
                    type="number"
                    value={edit.year ?? new Date().getFullYear()}
                    onChange={(e) => setEdit({ ...edit, year: Number(e.target.value) })}
                    className="h-10 w-full rounded-md border border-border bg-background px-3 text-sm"
                  />
                </label>
              </div>
