...
function BoardAdmin() {
  ...
  const save = async () => {
    ...
    const payload = {
      name: edit.name!,
      role: (edit.role ?? "editorial") as "editorial" | "advisory" | "reviewer",
      title: edit.title ?? null,
      affiliation: edit.affiliation ?? null,
    ...
    <>
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="mt-1 font-serif text-3xl font-semibold">Editorial board</h1>
        </div>
        <button
          onClick={() => setEdit({ role: "editorial", is_active: true, sort_order: rows.length })}
        ...
        {rows.map((m) => (
          <div key={m.id} className="rounded-xl border border-border bg-card p-4">
            ...
            <div className="mt-3 flex gap-1">
              <button onClick={() => setEdit(m)} className="rounded-md p-2 hover:bg-accent">
          ...
          >
            <div className="flex items-center justify-between">
              <h2 className="font-serif text-xl font-semibold">
                {edit.id ? "Edit member" : "New member"}
              </h2>
            ...
            <div className="mt-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <F
                  label="Affiliation"
                  value={edit.affiliation ?? ""}
                  onChange={(v) => setEdit({ ...edit, affiliation: v })}
                />
                <F
                  label="Country"
                  value={edit.country ?? ""}
