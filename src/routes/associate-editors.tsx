  ...
  head: () => ({
    meta: [
      { title: "Associate Editors — INSIGHTONIX" },
      {
        name: "description",
        content:
          "Meet the subject editors who handle submissions across finance, marketing, HR, operations, and strategy at INSIGHTONIX.",
      },
...
function AssociateEditors() {
  ...
  const editors = data.filter(
    (m) => m.role === "editorial" && !(m.title ?? "").toLowerCase().includes("editor-in-chief"),
  );
          ...
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              <h2 className="font-serif text-3xl font-semibold">
                {editors.length} subject editors · six continents
              </h2>
              <p className="mt-2 max-w-2xl text-muted-foreground">
                Editors are appointed for renewable three-year terms, chosen for their scholarly
                record, editorial temperament, and demonstrated commitment to double-blind review.
              </p>
            </div>
          </div>
