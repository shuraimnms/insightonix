  ...
  head: () => ({
    meta: [
      { title: "Editor-in-Chief — INSIGHTONIX" },
      {
        name: "description",
        content:
          "Meet the Editor-in-Chief leading the editorial direction and integrity of INSIGHTONIX.",
      },
...
function EiC() {
  ...
  const chief =
    data.find((m) => (m.title ?? "").toLowerCase().includes("editor-in-chief")) ??
    data.find((m) => m.role === "editorial") ??
    null;
      ...
      <div className="container-page py-12">
        ...
        {chief ? (
              ...
              <div className="mt-6 rounded-xl border border-border bg-card p-5 text-sm">
                ...
                {chief.email ? (
                  <a
                    ...
                    className="mt-2 inline-flex items-center gap-2 text-brand hover:underline"
                  >
            ...
            <article>
              ...
              <blockquote className="mt-8 rounded-xl border-l-4 border-brand bg-brand-muted/40 p-6">
                ...
                <p className="mt-3 font-serif text-lg leading-relaxed">
                  "Our job as editors is simple: protect the scholarly record and give every
                  rigorous, honest study a fair, timely hearing — regardless of where the author
                  sits or which theories are in fashion."
                </p>
          ...
          <div className="rounded-xl border border-dashed border-border bg-secondary/30 p-10 text-center text-sm text-muted-foreground">
            The Editor-in-Chief profile will be announced shortly. In the meantime, please browse
            our{" "}
            <Link to="/editorial-board" className="text-brand hover:underline">
