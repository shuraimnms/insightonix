...
const SITE_ID =
  import.meta.env.VITE_SITE_ID || window.location.hostname.split(".")[0].toUpperCase();
    ...
    try {
      ...
      if (res.data) {
        ...
        const keys = Object.keys(res).filter(
          (k) => k !== "pagination" && k !== "success" && k !== "error",
        );
...
export const supabase = {
  ...
  auth: {
    getSession: async () => {
      const token =
        typeof window !== "undefined" ? localStorage.getItem("universal_auth_token") : null;
